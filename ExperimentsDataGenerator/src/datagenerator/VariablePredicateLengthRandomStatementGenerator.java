...
public class VariablePredicateLengthRandomStatementGenerator extends RandomStatementGenerator{ 
	...
	int sizeOfVariablesSpace = 0;
	ArrayList<gqr.Predicate> targetSchema = null;
	
	public VariablePredicateLengthRandomStatementGenerator( ArrayList<gqr.Predicate> targetSchema, int sizeOfVariablesSpace, int start,int stop, int num_fun,int dist,int num_dup){
		this.targetSchema=targetSchema;
		this.sizeOfVariablesSpace =sizeOfVariablesSpace;
		...
		_variable_generator = new RegularRandomVariableGenerator(0,sizeOfVariablesSpace);
	...
	public Statement getRandomStatement(boolean use_all){
		...
		for (i = 0; i < NumberFunctions; i++){
			...
			predname = predname.substring(0, predname.lastIndexOf("00"+NumberFunctions));
...
//			}
			...
			Random r = new Random();
			int i1 = r.nextInt(targetRelationSize);
			int i2 = r.nextInt(targetRelationSize);
			while(i1 == i2)
				i2 = r.nextInt(targetRelationSize);
			
			for (j =0; j < targetRelationSize; j++){
				if(j == i1)
					apred.addVariable((new Integer(i)).toString());
				else if (j == i2)
					apred.addVariable((new Integer(i+1)).toString());
	...
	public String[] createPermutation(){
		String[] retval = new String[sizeOfVariablesSpace];
		int i,j;
		String a_val;
		boolean found;
		for (i = 0; i < sizeOfVariablesSpace; i++){
