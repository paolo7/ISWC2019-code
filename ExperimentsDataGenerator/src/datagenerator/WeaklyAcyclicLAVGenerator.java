...
public class WeaklyAcyclicLAVGenerator extends RandomStatementGenerator{ 
	...
	public WeaklyAcyclicLAVGenerator( int start,int stop, int num_fun,int length,int dist, int num_dup){
	...
	public static void main(String []args) throws CloneNotSupportedException{
		...
		//generateStarConstraintData();
		generateChaseConstraintData();

	}

	public static void generateStarConstraintData() throws CloneNotSupportedException{
		String dir = System.getProperty("user.dir")+"/constraint_data/starNew/";

		for(int i=0; i<20; i++) //WE ARE GOING TO CONSTRUCT this many DATASETS (EACH SET HAS 1 QUERY, 50 CONSTRAINTS, 500 VIEWS)
		{
			...
			StarRandomStatementGenerator a1 = new StarRandomStatementGenerator(1, 40, 5, 5, 3, 3);
			...
			queryFile.append(queryStatement.printString()).append(System.getProperty("line.separator"));
			queryFile.close();
			//END CREATE QUERY-------------------------------------------------------
			...
			for(int j=0; j<300; j++)
			{
				StarRandomStatementGenerator a = null; //new WeaklyAcyclicLAVGenerator(1, 20, 4, 4, 1, 3);
...
//				else
					a = new StarRandomStatementGenerator(1, 500/*(j>500)?500:((j<200)?200:j)*/, 5, 5, 1, 3);
				...
				{
					addConstraintInGraph(g,constraint);
			...
			viewFile.append("v"+queryStatement.printString()).append(System.getProperty("line.separator")); //add the query as one of the views

//			for(int j=0; j<201; j++)
//			{
//				StarRandomStatementGenerator a;
//				// 					if(j<81)
//				// 						a = new StarRandomStatementGenerator(1, 20, 4, 3, 5, 3);
//				// 					else
//				a = new StarRandomStatementGenerator(1, 200, 5, 5, 5, 3);
//
...
//			}
			...
			for(int j=0; j<1000; j++)
			{
				...
				StarRandomStatementGenerator a = new StarRandomStatementGenerator(1, 500, 5, 5, 5, 3);
				...
				viewFile.append(foo.printString()).append(System.getProperty("line.separator"));
			}
			viewFile.close();
...
//	public static void generateChaseConstraintData() throws CloneNotSupportedException{
//		String dir = System.getProperty("user.dir")+"/constraint_data/chainSameViewsAsConstraintsExceptOnePredicateMoreRewritings2";
//
//		for(int i=0; i<20; i++) //WE ARE GOING TO CONSTRUCT this many DATASETS (EACH SET HAS 1 QUERY, 50 CONSTRAINTS, 500 VIEWS)
//		{
//			System.out.println(new File(dir+"/run_"+i).mkdir());
...
//			for(int j=0; j<300; j++)
//			{
//				WeaklyAcyclicLAVGenerator a = null; //new WeaklyAcyclicLAVGenerator(1, 20, 4, 4, 1, 3);
//
//				//				if(j<21)
...
//									if(j%10 == 1)
//				 						a = new WeaklyAcyclicLAVGenerator(1, 60, 4, 4, 1, 3);
//				 					else
//				 						a = new WeaklyAcyclicLAVGenerator(1, 300, 4, 4, 1, 3);
//
//				Statement constraintStatement = a.getRandomStatement(false);
//				constraintStatement.getHead().setFunctionHead("c"+j);
//
...
//			{
//
//				WeaklyAcyclicLAVGenerator a = null;//new WeaklyAcyclicLAVGenerator(1, 500, 8, 4, 3, 5);
//
//				//				if(j<21)
...
//				//				else
//				a = new WeaklyAcyclicLAVGenerator(1, 300, 5, 4, 4, 3);
//
//				Statement foo = a.getRandomStatement(false);
//				foo.getHead().setFunctionHead("v"+j);
//
...
//	}
	...
	public static void generateChaseConstraintData() throws CloneNotSupportedException{
		String dir = System.getProperty("user.dir")+"/chasebenchData_chain/chainSameViewsAsConstraintsExceptOnePredicateMoreRewritings3";

		for(int i=2; i<20; i++) //WE ARE GOING TO CONSTRUCT this many DATASETS (EACH SET HAS 1 QUERY, 50 CONSTRAINTS, 500 VIEWS)
		{
			System.out.println(new File(dir+"/run_"+i).mkdir());
			Graph g = new Graph();
			...
			for(int j=0; j<300; j++)
			{
				...
				//						a = new WeaklyAcyclicLAVGenerator(1, 20, 4, 4, 1, 3);
									if(j%10 == 1)
				 						a = new WeaklyAcyclicLAVGenerator(1, 60, 3, 4, 1, 1);
				 					else
				 						a = new WeaklyAcyclicLAVGenerator(1, 300, 3, 4, 1, 1);
...
//					}
			...
			for(int j=0; j<1000; j++)
			{
				...
				//						a = new WeaklyAcyclicLAVGenerator(1, 500, 4, 4, 4, 3);
				//				else
				a = new WeaklyAcyclicLAVGenerator(1, 300, 3, 4, 4, 2);
	...
	static void addConstraintInGraph(Graph g, String constraint) {
		...
		Pair<SourcePredicateJoin,List<SourcePredicateJoin>> constraintPJs = GQR.createConstraintPJs(con);
		SourcePredicateJoin antecedent  = constraintPJs.getA();
		setOriginallyExistentialVariables(antecedent,constraintPJs.getB());
	...
	private static void setOriginallyExistentialVariables(PredicateJoin antecedent, List<SourcePredicateJoin> list) {

		for(PredicateJoin pj: list)
		{
			...
			for(int i=1; i<=map.size(); i++)
			{
				...
				for(JoinDescription jd : jv.getJoinDescriptions())
				{
					...
					if(jd.getPredicate().equals(antecedent.getPredicate()))
					{
